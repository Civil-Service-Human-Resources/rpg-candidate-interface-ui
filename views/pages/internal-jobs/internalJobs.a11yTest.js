...
describe('Internal Jobs Page', () => {
    beforeEach((done) => {
        driver = new selenium.Builder().forBrowser('chrome');
        browser = driver.build();
        browser.manage().timeouts().setScriptTimeout(60000);

        browser.get('http://test:test@localhost:3000/internal-jobs').then(() => {
    ...
    it('should fetch the Internal Jobs page and analyze it', (done) => {
    ...
    fit('should accept an incorrect email and analyze response', (done) => {
        browser.findElement(selenium.By.id('email')).sendKeys('grant@me.com')
            .then(() => {
                browser.findElement(selenium.By.className('button')).click()
                    .then(() => {
                        browser.findElement(selenium.By.tagName('body'))
                            .then(() => {
                                AxeBuilder(browser).analyze((results) => {
