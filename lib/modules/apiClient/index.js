...
apiClient.vacancy = {
    ...
    getById: (vendorid, id, next) => request(API_SEARCH_URL, `/vacancy/${vendorid}/${id}`, 'GET', {}, next),
    getMeta: next => request(API_SEARCH_URL, '/vacancy/vacancymetadata', 'GET', {}, next),
};
...
apiClient.internalJobs = {
    authorize: (email, department, next) => {
        const reqOptions = {
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                emailAddress: email,
...
async function request(url, path = null, method = 'GET', requestOptions = {}, next) {
    ...
    try {
        return await fetch(`${url}${path}`, options)
            ...
            .then(data => (data.status === 204 ? true : data.json()))
