...
const { doesDepartmentLogoExist, getDepartmentLogos } = require('../department');
...
const fetchVacancyList = async function ({
    ...
    const overseasJob = !filtered || (filtered && overseas) ? 'true' : 'false';
    ...
    const { content, ...other } = await response.json();
    const vacancies = formatResultsData(content, logos);
...
const fetchVacancyDetails = async function (id, next) {
    ...
    return formatVacancyData(data, logos);
};

function formatVacancyData(vacancy = {}, logos = []) {
    const salary = returnSalaryDetails(vacancy.salaryMin, vacancy.salaryMax, vacancy.overrideSalary);
    const closingDate = gdsDateFormat(vacancy.closingDate);
    const showCscInfo = vacancy.displayCscContent && vacancy.publicOpeningDate;
    const disabilityLogo = generateDisabilityLogoInfo(vacancy.department.disabilityLogo);
    ...
    const hasLogo = doesDepartmentLogoExist(logoFilename, logos);
